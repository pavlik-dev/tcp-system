...
def L(m=B,t=object):
	while A:
		try:return t(D(m))
		except ValueError:print('The data you entered cannot be converted to '+t.__name__+'.');continue
...
class M:
	...
	def rh(C):
		while True:
			D=C.sock.recvfrom(4096);E=D[0].decode()
			print(B.join([A if A in I.printable else B for A in E]))
			C.serv_resp=A
N=D('IP:')
O=L('Port:',int)
E=M(N,O)
